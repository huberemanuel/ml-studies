...
class MyLoss(nn.Module):
    ...
    def __init__(self, l: float = 0):
        super().__init__()
        self.l = l

    def forward(self, y_pred, y_true, params):
        loss = (y_pred - y_true) ** 2
        loss = loss.mean()
        params = torch.sqrt((parameters_to_vector(params) ** 2).sum())
        loss += self.l * params.item()
        return loss
...
class MyTrainer:
    ...
    def fit(self, train_loader, epoch):
        ...
        for batch, (b_x, b_y) in enumerate(train_loader):
            ...
            loss = self.loss_fn(y_pred, b_y, model.parameters())
